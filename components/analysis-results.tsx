    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h2 className="mb-1 text-2xl font-bold text-foreground">{t("analysis.title")}</h2>
            {data.product_name && (
              <p className="mb-1 text-sm text-muted-foreground">{t("analysis.product")}: {data.product_name}</p>
            )}
            {typeof data.product_price === 'number' && (
              <p className="mb-1 text-sm text-muted-foreground">{t("analysis.price")}: ${data.product_price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
            )}
            {typeof data.product_rating === 'number' && (
              <p className="text-sm text-muted-foreground">{t("analysis.rating")}: {data.product_rating.toFixed(1)} / 5.0</p>
            )}
            <p className="text-muted-foreground">{t("analysis.basedOn")} {data.total_reviews} {t("analysis.reviews")}</p>
          </div>
        </div>
      </div>
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="mb-2 text-sm font-medium text-muted-foreground">{t("analysis.overall")}</p>
          ...
          <div className="text-right">
            <p className="mb-2 text-sm font-medium text-muted-foreground">{t("analysis.score")}</p>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="p-6">
          ...
          <p className="text-sm text-muted-foreground">
            {data.positive_count + data.neutral_count + data.negative_count > 0
              ? ((data.positive_count / (data.positive_count + data.neutral_count + data.negative_count)) * 100).toFixed(1)
              : '0.0'}%
          </p>
        </Card>
        ...
        <Card className="p-6">
          ...
          <p className="text-sm text-muted-foreground">
            {data.positive_count + data.neutral_count + data.negative_count > 0
              ? ((data.neutral_count / (data.positive_count + data.neutral_count + data.negative_count)) * 100).toFixed(1)
              : '0.0'}%
          </p>
        </Card>
        ...
        <Card className="p-6">
          ...
          <p className="text-sm text-muted-foreground">
            {data.positive_count + data.neutral_count + data.negative_count > 0
              ? ((data.negative_count / (data.positive_count + data.neutral_count + data.negative_count)) * 100).toFixed(1)
              : '0.0'}%
          </p>
        </Card>
      </div>
      ...
      {/* Comparativa de precios (si aplica) */}
